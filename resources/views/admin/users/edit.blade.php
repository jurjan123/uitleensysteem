...
<div class="shadow-md rounded-lg overflow-hidden flex flex-col gap-x  p-7 bg-gray-100 col-span-4 leading-10 w-4/6 min-h-fit">
    ...
    <div>
        <p>Voornaam</p>
        <input type="text" class="rounded-md w-11/12 mb-5 pl-2" name="first_name" value="{{old("first_name",$user->first_name)}}">
        @error("first_name")
    ...
    <div>
        <p>Tussenvoegsel </p>
        <input type="text" class="rounded-md mb-5 pl-2 w-11/12" name="preposition"  value="{{old("preposition",$user->preposition)}}">
        @error("preposition")
        <p class="text-red-500 text-xs -mt-3">{{$message}}</p>
        @enderror
    </div>
    <div>
        <p>Achternaam</p>
        <input type="text" class="rounded-md mb-5 pl-2 w-11/12" name="last_name"  value="{{old("last_name", $user->last_name)}}">
        @error("last_name")
        <p class="text-red-500 text-xs -mt-3">{{$message}}</p>
        @enderror
    </div>
    <div>
        <p>Email</p>
        <input type="email" class="rounded-md mb-5 pl-2  w-11/12" name="email"  value="{{old("email", $user->email)}}">
        @error("email")
        <p class="text-red-500 text-xs -mt-3">{{$message}}</p>
        @enderror
    </div>
